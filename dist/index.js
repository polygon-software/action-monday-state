...
async function main() {
  ...
  const boardId = await src_monday.boardByItem(itemId);
  core.debug(`Found board corresponding to Item. Board-iD is: ${boardId}`)

  const columnId = statusColumnId || await src_monday.columnIdByTitle(boardId, statusColumnTitle);
  core.debug(`Found Column ID: ${columnId}`)

  const newStatus = await src_monday.updateItemStatus(itemId, boardId, columnId, status);
